...
function CitationReaderApp({ password, onLogout }: { password: string; onLogout: () => void }) {
  ...
  // キャンセル直後のクールダウン。
  // Cloudflare Workersのログで、1 回のキャンセル後に 2 秒間隔で自動再送信が 4 回
  // 発生するケースを観測した (ユーザー連打orブラウザのfetch retry由来)。
  // isStreaming=falseへの切替直後にも再送信を遮断するため、2 秒のロック期間を設ける。
  ...
  const handleSubmit = useCallback(async () => {
    // 二重送信ガード: canSubmitと重複するが、念のためロジック層でも遮断。
    // ブラウザのfetch retry等でhandleSubmitが意図せず呼ばれるケースを防ぐ。
    ...
    try {
      ...
      // ネットワーク失敗等の例外はraw messageを出さず、汎用文言で表示する
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={onLogout}
              // 44pxタッチターゲット + WCAG AAコントラスト (slate-700/300)
