 ...
 * per-isolate制約 (最悪N × MAX_REQUESTSまで通る) を解消する。
 *
 * 動作モード:
 *   1. Cloudflare Workers本番環境: `getCloudflareContext().env.RATE_LIMITER` が
 *      存在し、`limit({ key })` 経由でisolate跨ぎのsliding windowを取得できる
 *   2. Workers Cache API: bindingが許可した後、またはbindingが無い本番環境で、
 *      同一edge内のburstを補助的に抑える
 *   3. dev / Vitest / binding / Cache API未設定環境: in-memory
 *      `checkRateLimit` (rate-limit.ts) に自動fallbackする
 *
 * 上位 (route.ts) は `checkRateLimitFromContext(...)` を呼ぶだけで、本番 / devの
 * 切替を意識しなくてよい。
 *
 * binding設定: `wrangler.jsonc` の `ratelimits` で
 * name=RATE_LIMITER / simple.limit=10 / simple.period=60 を宣言済。
 * namespace_idはCloudflareアカウント内で一意な正整数文字列を使う。
 */
...
/**
 * Cloudflare Rate Limiting binding APIのミニマル型。
 * 本物のSDK型を直接参照するとbuild-timeにWorkers型が必要になるため、
 * 必要部分だけ手書きで宣言。
 */
...
/**
 * 任意のCloudflare-like envオブジェクトからRATE_LIMITER bindingを取り出す。
 * binding不在 / 型不一致時はnullを返し、呼び出し側はin-memory fallbackする。
 */
...
/**
 * bindingがある環境ではbinding経由で判定、なければin-memoryにfallback。
 *
 * @param env - getCloudflareContext().env等のCloudflare bindingsコンテナ
 * @param ip - getClientIp() の戻り値
 * @returns RateLimitResult。binding経由のときremainingは不明なので 0 を返す
 *          (上位はallowedとretryAfterSecondsだけ参照する設計)
 */
export async function checkRateLimitFromContext(
  ...
  try {
    ...
    // binding側でblocked。Cloudflare Rate Limiting bindingはretry-afterを
    // 直接返さないため、wrangler.jsoncのsimple.period (60s) を上限とした
    // 概算値を使う (sliding windowの最大残時間)。
    return { allowed: false, remaining: 0, retryAfterSeconds: 60 };
  } catch (err) {
    // binding呼び出しが失敗した場合は安全側でin-memoryにfallbackして
