// Anthropic SDK 0.90 のcitations_delta由来raw payloadを、本アプリ用の `CitationLocation` 型に
// 正規化する変換関数。
//
// SDK由来payloadをruntime validationしてからアプリ用の型に正規化する。
// - SDKの `delta.citation` は `unknown` 由来の構造で、ランタイムに以下のような複数のtypeを取る
//   (char_location / page_location / content_block_location)。
// - 受け取り側は `CitationLocation` 型を期待するため、以前は `as unknown as CitationLocation` で
//   逃げていたが、shapeが変わったときに気付けない。本変換関数で形を最低限検査して、
//   不整合はnullを返す。
...
/**
 * 任意の入力 (SDKのcitations_delta.citation) をCitationLocationに正規化する。
...
export function toCitationLocation(input: unknown): CitationLocation | null {
  ...
  // 整数 + 非負を要求。負のdocument_indexはSDK仕様にない値で、optionalのchar/page/block indexと
  // 整合性を取るため拒否する。
  if (typeof document_index !== 'number' || !Number.isInteger(document_index) || document_index < 0)
    return null;

  // 任意フィールドは存在すれば型を確認、無ければundefinedのまま通す
  ...
  // narrowing後のnumber | undefinedを局所変数で確定 (lintのno-unnecessary-type-assertion回避)
