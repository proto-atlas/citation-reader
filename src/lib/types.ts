 ...
 *
 * raw upstream errorsをUIに出さない / OWASP Improper Error Handling
 * への対応として、SDKや内部処理の生エラーをUIに出さない方針。codeのみをSSE / JSONで
 * 返却し、サーバ内ではconsole.errorで詳細を残す。
 */
...
/**
 * 非streamingエンドポイント (/api/auth, JSON系) の標準エラー形式。
 */
