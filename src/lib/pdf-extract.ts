...
/** pdfjsのTextItemから拾う最小限の形。テスト容易化のためnarrowな型で受ける。 */
...
/**
 * 1 ページ分のTextItem群を 1 文字列に結合する。
 * 区切りを " " にすると日本語PDFのルビや文字単位分割で余計な空白が入り
 * 「令 れ い 和 わ」のように崩れるため、連結はセパレータ無しで行う。
 * 行末はhasEOLで判定して改行を入れる（英語PDFの単語間空白はitem.str側に含まれている）。
 */
...
// Anthropic入力上限 (200,000 文字) とOpenNext / Workersのメモリ (128 MiB) を踏まえ
// 過大なPDFをクライアント側で先に弾く。UIで表示する閾値とこの定数で同期する。
export const MAX_PDF_SIZE_BYTES = 10 * 1024 * 1024; // 10 MiB
export const MAX_PDF_PAGES = 50;

export class PdfTooLargeError extends Error {
  constructor(public readonly limitMib: number) {
    super(`PDFが大きすぎます (上限 ${limitMib} MiB)`);
...
export class PdfTooManyPagesError extends Error {
  constructor(
    ...
    super(`PDFのページ数が多すぎます (${pages} ページ、上限 ${limit} ページ)`);
