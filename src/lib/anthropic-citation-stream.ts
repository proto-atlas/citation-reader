...
// 回答内のMarkdownがそのまま表示されることを避ける:
// UI側はstreaming + 引用バッジinline挿入のためにplain textレンダリングをしている。
// react-markdown等を入れると引用indexとの同期が複雑化するため、AI側に
...
const SYSTEM_PROMPT = [
  ...
  '',
  '【出力フォーマット】回答は必ずプレーンテキストで返してください。Markdown記法 (** ## - * 等) や箇条書きの記号は使わず、自然な文章で書いてください。改行は段落単位で最低限のみ使い、太字や見出しの装飾は不要です。',
  '',
