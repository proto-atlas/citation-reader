...
describe('toCitationLocation', () => {
  it('null / undefined / 文字列 / numberなどobjectでない値でnullを返す', () => {
  ...
  it('document_indexがintegerでない場合nullを返す', () => {
    expect(
      toCitationLocation({
        type: 'char_location',
        cited_text: 'hello',
  ...
  it('document_indexが負の数ならnullを返す', () => {
    expect(
      toCitationLocation({
        type: 'char_location',
        cited_text: 'hello',
  ...
  it('char_locationの最小フォーマットを受理する', () => {
  ...
  it('content_block_locationを受理する', () => {
    const result = toCitationLocation({
  ...
  it('start_char_indexが負の数ならnullを返す (validation)', () => {
    expect(
      toCitationLocation({
        type: 'char_location',
        cited_text: 'hello',
  ...
  it('document_titleがnullの場合はnullを保持する (Anthropic SDKの互換性)', () => {
    const result = toCitationLocation({
      type: 'char_location',
      cited_text: 'hello',
      document_index: 0,
