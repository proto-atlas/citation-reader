...
export function AnswerView({
  ...
  if (blocks.length === 0 && !isStreaming) {
    return (
      // text-slate-400 はライト背景でコントラスト不足 (axe-core検出)。
      // 600/400 でWCAG AA達成。
        ...
        <div className="prose prose-slate dark:prose-invert max-w-none text-[15px] leading-relaxed text-slate-900 dark:text-slate-100">
          {blocks.map((block) => (
            <span key={block.index}>
              {/* Markdown記法 (`**bold**` / `*italic*` / `` `code` ``) を
                  inline要素にレンダリング。引用バッジはtextと並列に挿入する
                  既存構造を保つので、citation同期は壊れない (markdown-inline.test.ts
                  でレンダリング順序を固定)。SYSTEM_PROMPTのMarkdown禁止指示と
                  二重防御。 */}
        ...
        <div className="flex flex-col gap-2">
          ...
          <ul className="flex flex-col gap-2">
            {flatCitations.map(({ key, n, citation }) => {
              const active = hoveredCitationKey === key;
              return (
                // li自体はrole不変 (リスト構造の意味は残す)、操作可能要素は内側のbutton。
                // 引用元リストはキーボード操作にも対応する。
                // li onClickからbuttonに変更してキーボード操作 (Enter / Space) と
                // タッチ 44px高さを確保する。
