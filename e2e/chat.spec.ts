...
test.describe('チャット (SSEモック)', () => {
  test('サンプル読込 → 質問する → 回答 + 引用バッジ表示 → クリックで原文プレビュー', async ({
    page,
  }) => {
    // /api/chatをPlaywright route() でモック。実Anthropic APIを叩かず課金させない。
    ...
    // サンプル読込ボタン (textareaにサンプル文字列が流し込まれる)
    await page.getByRole('button', { name: 'サンプル読込' }).click();
    // textareaに「エッジコンピューティング」で始まる本文が入っていることを確認
