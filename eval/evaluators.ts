 ...
 * LLM出力品質の評価関数群。
 *
 * `/api/chat` のSSE出力を集約した「回答テキスト + 引用配列」を
 * 入力に取り、以下 3 つの評価軸で品質を機械的に検査する。
 *
 *   1. Markdown混入: 回答テキストに `**bold**` / `# heading` / ` ``` ` などが残っていないか
 *   2. 言語一致: 質問が日本語なら回答も日本語、英語なら英語かどうか
 *   3. 引用件数 / 引用cited_textの重複
 *
 * すべて純関数なのでVitestで単体テスト可能。実Anthropic API呼び出しは
...
/**
 * 回答テキストにMarkdown記法が混入していないか検査する。
 * SYSTEM_PROMPTにMarkdown禁止を明記しているが、LLM出力は非決定的
 ...
 * 文字種ベースの簡易判定 (CJK比率 > 0.05 ならja、ASCII比率 > 0.95 ならen)。
 * 厳密な言語検出ではないが、ja/enの取り違えは検出できる。
 */
...
/**
 * 引用配列の件数と重複率を集計する。
 * citation 0 件の回答や同一cited_textを多重引用する回答を検出できる。
 */
 ...
 * runner.mjsはこれをsample-cases.jsonの各ケースに対して実行し集計する。
 */
...
export function evaluateTurn(turn: ChatTurnSummary): TurnEvalReport {
  ...
  // 合格条件: Markdown漏れなし + 言語一致 + 引用 1 件以上 + 重複率 50% 未満
  const passed =
